...
namespace lightdb {
    namespace utility {
        ...
        void BitArray::Replace(const size_t start, const size_t end, const vector<bool> &replacement) {
            ...
            std::copy(this->begin(), this->begin() + start, replaced.begin());
            // Insert the entirety of the replacement
            std::copy(replacement.begin(), replacement.end(), replaced.begin() + start);
            // Insert the chunk starting at end
            std::copy(this->begin() + end, this->end(), replaced.begin() + start + replacement.size());

            *this = replaced;
        }

        void BitArray::ByteAlign() {
            auto extend = CHAR_BIT - this->size() % CHAR_BIT;
