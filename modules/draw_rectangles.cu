...
void draw_rectangles(
    ...
    if(rectangle_id < rectangle_count && im_x < width && im_y < height) {
        ...
        const bool on_left_border =   global_im_x >= b.x &&
                                      global_im_x < b.x + line_width &&
                                      global_im_y >= b.y  &&
                                      global_im_y <= b.y + b.height;
        const bool on_right_border =  global_im_x > b.x + b.width - line_width &&
                                      global_im_x <= b.x + b.width &&
                                      global_im_y >= b.y &&
                                      global_im_y <= b.y + b.height;

        const bool on_top_border =    global_im_y >= b.y &&
                                      global_im_y < b.y + line_width &&
                                      global_im_x >= b.x + line_width &&
                                      global_im_x <= b.x + b.width - line_width;
        const bool on_bottom_border = global_im_y > b.y + b.height - line_width &&
                                      global_im_y <= b.y + b.height &&
                                      global_im_x >= b.x + line_width &&
                                      global_im_x <= b.x + b.width - line_width;
