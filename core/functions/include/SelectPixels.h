    ...
    public:
        ...
        shared_reference<LightField> operator()(LightField &input) override {
            ...
            for (auto &frame: data.frames()) {
                ...
                for (unsigned int y = minAndMax.minY; y <= minAndMax.maxY; y++) {
...
//                // Instead, we want to set all pixels that are not in the boxes to black.
...
//
////                CHECK_EQ(cuMemsetD2D8(cu da->handle() + uv_offset,
////                        cuda->pitch(),
////                        0,
////                        cuda->width(),
...
//    };
public:
    SelectPixels() : functor::unaryfunctor("SelectPixels", CPU()) { }
};
