...
namespace lightdb {
    ...
    // does bitset use move?
    // should i use bool vector?
    // should i use split instead of manually?
  //todo: fix emulation addition to add the nal marker if possible
    ...
    BitArray RemoveEmulationPrevention(const bytestring &data, const unsigned long start, const unsigned long end) {
        ...
        size_t set_size = (data.size() - emulation_indices.size()) * CHAR_BIT;
    ...
    bytestring AddEmulationPreventionAndMarker(const BitArray data, const unsigned long start, const unsigned long end) {
        ...
        unsigned long set_size = data_size + emulation_indices.size() + nal_marker.size();
        bytestring bytes;
        bytes.reserve(set_size);
        ...
        for (auto i = 0; i < data_size; i++) {
            ...
            if (!emulation_indices.empty() && i == emulation_indices.front()) {
                emulation_indices.pop_front();
                bytes.push_back(three);
            }
            auto curr = data.GetByte(i);
