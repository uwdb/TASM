...
static std::unordered_map<std::string, std::vector<std::string>> videoToObjectsToCrackOn({
      ...
      {"oldtown", {"car"}},
      {"seeking", {"person"}},
      {"tennis", {"person", "sports_ball", "tennis_racket"}},
      ...
      {"traffic-2k-001", {"car", "pedestrian"}},
      {"traffic-4k-002", {"car", "pedestrian"}},
      {"traffic-4k-000", {"car", "pedestrian"}},
      {"car-pov-2k-001-shortened", {"car", "pedestrian"}},
      {"car-pov-2k-000-shortened", {"car", "pedestrian"}},
      {"traffic-4k-002-ds2k", {"car", "pedestrian"}},
      {"cosmos", {"person", "sheep"}},
      {"elfuente_full", {"person"}},
});
...
TEST_F(VisitorTestFixture, testGetNumFramesForNoTiles) {
...
//            "oldtown",
    ...
    std::vector<std::vector<std::string>> queryObjectsList {
...
//        {"traffic light"},
    ...
    for (const auto &video : videos) {
        auto gopLength = videoToFramerate.at(video);
        auto dimensions = videoToDimensions.at(video);
        for (const auto &objects : queryObjectsList) {
            PixelMetadataSpecification selection("labels", MetadataElementForObjects(objects));
            Workload workload(video, {selection}, {1});

            auto singleTileConfiguration = std::make_shared<tiles::SingleTileConfigurationProvider>(dimensions.first, dimensions.second);
            WorkloadCostEstimator costEstimator(singleTileConfiguration, workload, gopLength, 0, 1, 0);

            unsigned int sawMultipleLayouts = 0;
            auto costElements = costEstimator.estimateCostForQuery(0, sawMultipleLayouts);
            std::cout << "Video " << video
                    << "\nquery-object " << combineStrings(objects)
                    << "\nnum-frames " << costElements.numTiles
                    << "\nnum-pixels " << costElements.numPixels << std::endl;
        }
    }

}

TEST_F(VisitorTestFixture, testNumDecodedFrames) {
    std::string video("traffic-2k-001");
    std::string object("pedestrian");
...
TEST_F(VisitorTestFixture, testScanAndSave) {
    auto input = Load("/home/maureen/lightdb-wip/cmake-build-debug-remote/test/resources/market_all/1-0-stream.mp4",
            Volume::limits(),
            GeometryReference::make<EquirectangularGeometry>(EquirectangularGeometry::Samples()));
    Coordinator().execute(input.Store("market-all-2"));
}

struct CrackingInfo {
    CrackingInfo(CrackingStrategy s, int d, std::string object)
        : strategy(s), duration(d), queryObject(object)
    {}
...
TEST_F(VisitorTestFixture, testCrackParticularCasesBasedOnMetadata) {
    std::unordered_map<std::string, std::vector<CrackingInfo>> videoToCrackingInfo{
            {"birdsincage", {CrackingInfo(CrackingStrategy::SmallTiles, 120, "bird")}},
//            {"car-pov-2k-000-shortened", CrackingInfo(CrackingStrategy::GroupingExtent, -1, "pedestrian")},
//            {"car-pov-2k-001-shortened", CrackingInfl(CrackingStrategy::GroupingExtent, -1, "pedestrian")},
            {"cosmos", {CrackingInfo(CrackingStrategy::SmallTiles, 24, "person"),
                        CrackingInfo(CrackingStrategy::GroupingExtent, 24, "sheep")}},
            {"elfuente1", {CrackingInfo(CrackingStrategy::SmallTiles, 120, "person")}},
            {"elfuente_full", {CrackingInfo(CrackingStrategy::SmallTiles, 60, "person")}},
            {"market_all", {CrackingInfo(CrackingStrategy::SmallTiles, 300, "car"),
                            CrackingInfo(CrackingStrategy::SmallTiles, 60, "person"),
                            CrackingInfo(CrackingStrategy::SmallTiles, 120, "truck")}},
            {"narrator", {CrackingInfo(CrackingStrategy::GroupingExtent, 120, "car"),
                          CrackingInfo(CrackingStrategy::SmallTiles, 60, "person")}},
            {"river_boat", {CrackingInfo(CrackingStrategy::SmallTiles, 60, "boat"),
                            CrackingInfo(CrackingStrategy::SmallTiles, 60, "person")}},
            {"seeking", {CrackingInfo(CrackingStrategy::GroupingExtent, 75, "person")}},
            {"square_with_fountain", {CrackingInfo(CrackingStrategy::SmallTiles, 60, "bicycle"),
                                      CrackingInfo(CrackingStrategy::GroupingExtent, 60, "person")}},
            {"street_night_view", {CrackingInfo(CrackingStrategy::SmallTiles, 60, "car"),
                                   CrackingInfo(CrackingStrategy::SmallTiles, 60, "person")}}
//            {"MOT16-01", CrackingInfo(CrackingStrategy::GroupingExtent, 30)},
//            {"MOT16-02", CrackingInfo(CrackingStrategy::SmallTiles, 30)},
//            {"MOT16-03", CrackingInfo(CrackingStrategy::GroupingExtent, 30)},
//            {"MOT16-04", CrackingInfo(CrackingStrategy::GroupingExtent, 60)},
//            {"MOT16-07", CrackingInfo(CrackingStrategy::GroupingExtent, -1)},
//            {"MOT16-08", CrackingInfo(CrackingStrategy::GroupingExtent, -1)},
//            {"MOT16-09", CrackingInfo(CrackingStrategy::GroupingExtent, -1)},
//            {"MOT16-10", CrackingInfo(CrackingStrategy::GroupingExtent, 90)},
//            {"MOT16-11", CrackingInfo(CrackingStrategy::GroupingExtent, 60)},
//            {"MOT16-12", CrackingInfo(CrackingStrategy::SmallTiles, 150)},
//            {"MOT16-13", CrackingInfo(CrackingStrategy::SmallTiles, 25)},
//            {"MOT16-14", CrackingInfo(CrackingStrategy::GroupingExtent, 125)},
    };

    for (auto it = videoToCrackingInfo.begin(); it != videoToCrackingInfo.end(); ++it) {
        auto video = it->first;
        for (const auto &crackingInfo : it->second) {
            auto object = crackingInfo.queryObject;
            auto input = Scan(video);
            MetadataSpecification metadataSelection("labels", std::make_shared<SingleMetadataElement>("label", object));
            std::string crackingStr =
                    crackingInfo.strategy == CrackingStrategy::SmallTiles ? "-smalltiles-" : "-grouping-extent-";
            std::string durationStr =
                    crackingInfo.duration == -1 ? "entire-video" : "duration" + std::to_string(crackingInfo.duration);
            std::string savedName = video + "-cracked-" + object + crackingStr + durationStr;
            std::cout << "Cracking " << video << " to " << savedName << std::endl;
            Coordinator().execute(input.StoreCracked(savedName, video, &metadataSelection,
                                                     crackingInfo.duration == -1 ? INT_MAX : crackingInfo.duration,
                                                     crackingInfo.strategy));
        }
    }
}
...
TEST_F(VisitorTestFixture, testCrackBasedOnMetadata) {
    ...
    std::vector<std::string> labels{"MOG", "MOG2"};

    for (const auto &label : labels) {
        auto input = Scan(video);
        MetadataSpecification metadataSelection("labels", std::make_shared<SingleMetadataElement>("label", label));
        std::string savedName = video + "-cracked-" + label + "-smalltiles-duration" + std::to_string(baseFramerate);
        Coordinator().execute(
                input.StoreCracked(savedName, video, &metadataSelection, baseFramerate, CrackingStrategy::SmallTiles));
    }
}
...
//struct EncodingInfo{
//    std::string video;
//    std::vector<std::string> crackingObjects;
//    CrackingStrategy crackingStrategy;
//    unsigned int duration;
//
//    EncodingInfo(const std::string &video, const std::vector<std::string> &crackingObjects, CrackingStrategy crackingStrategy, unsigned int duration)
//            : video(video), crackingObjects(crackingObjects), crackingStrategy(crackingStrategy), duration(duration)
//    {}
//};
    ...
    for (const auto &video : videos) {
        for (const auto &crackingObjects : objectsToTileAround) {
            ...
            if (crackingObjects.front() == "all_objects") {
                metadataElement = std::make_shared<AllMetadataElement>();
            } else {
                std::vector<std::shared_ptr<MetadataElement>> componentElements(crackingObjects.size());
                std::transform(crackingObjects.begin(), crackingObjects.end(), componentElements.begin(),
                               [](auto obj) {
                                   return std::make_shared<SingleMetadataElement>("label", obj);
                               });
                metadataElement = componentElements.size() == 1 ? componentElements.front() : std::make_shared<OrMetadataElement>(componentElements);
            }
            MetadataSpecification metadataSpecification("labels", metadataElement);
            ...
            std::cout << "Tile-strategy small-dur-" << duration << std::endl;
            Coordinator().execute(
                    input.StoreCracked(savedName, video, &metadataSpecification, duration, crackingStrategy));
        }
    }
...
//            "tennis"
    ...
    std::vector<std::string> crackingStrategies{"YOLOv3-0.2", "YOLOv3-0.5"};
//    std::vector<std::string> crackingStrategies{"all_objects"};

    for (const auto &video : videos) {
        auto objects = videoToObjectsToCrackOn.at(video);
        auto framerate = videoToFramerate.at(video);
        for (const auto &crackingStrategy : crackingStrategies) {
            auto catalogEntry = video + "-cracked-" + crackingStrategy + "-smalltiles-duration" + std::to_string(framerate);
            for (const auto &object : objects) {
                PixelMetadataSpecification selection("labels",
                                                     std::make_shared<SingleMetadataElement>("label", object));
                for (int i = 0; i < 3; ++i) {
                    std::cout << "*** Reading from " << catalogEntry << std::endl;
                    std::cout << "\nVideo " << video << "\nCracking-object " << crackingStrategy << "\nQuery-object "
                              << object << "\nTile-strategy small-dur-" + std::to_string(framerate) << std::endl;
                    bool usesOnlyOneTile = false;
                    std::cout << "Uses-one-tile " << usesOnlyOneTile << std::endl;
...
TEST_F(VisitorTestFixture, testFailureCase) {
    std::string queryObject("pedestrian");
    std::string video("traffic-4k-002-ds2k");
...
//    std::vector<std::string> uniformTileSuffixes{"-2x2", "-3x3", "-4x4", "-5x5", "-6x6", "-7x7", "-7x8", "-7x9", "-7x10"};
    ...
    for (const auto& video : videos) {
        for (const auto &queryObjects : queryObjectsList) {
            std::shared_ptr<MetadataElement> metadataElement = MetadataElementForObjects(queryObjects);
            auto queryObject = combineStrings(queryObjects);
//            std::vector<std::shared_ptr<MetadataElement>> componentElements(queryObjects.size());
//            std::transform(queryObjects.begin(), queryObjects.end(), componentElements.begin(),
//                           [](auto obj) {
//                               return std::make_shared<SingleMetadataElement>("label", obj);
//                           });
//            metadataElement = componentElements.size() == 1 ? componentElements.front() : std::make_shared<OrMetadataElement>(componentElements);
            PixelMetadataSpecification selection("labels", metadataElement);
            PixelMetadataSpecification tileSelection("labels", MetadataElementForObjects({"car", "truck", "bus", "motorbike", "person", "traffic light"}));
//            auto queryObject = combineStrings(queryObjects);
            std::cout << "\n***object," << queryObject << std::endl;

            auto baseFramerate = videoToFramerate.at(video);

//                std::shared_ptr<MetadataElement> metadataElement = std::make_shared<SingleMetadataElement>("label",
//                                                                                                           queryObject);
...
//                        PixelMetadataSpecification tileSelection("labels", std::make_shared<SingleMetadataElement>("label", crackingObject));
...
//            }
            ...
            std::vector<std::string> crackingObjects{combineStrings({"car", "truck", "bus", "motorbike", "person", "traffic light"})}; //, "YOLOv3-tiny-0.02"
...
TEST_F(VisitorTestFixture, testCompareTilings) {
    std::string video("Netflix_FoodMarket2");

    std::vector<std::string> suffixes{
        "-7x7",
        "-cracked-person-smalltiles-duration60",
    };

    auto metadataSelection = std::make_shared<SingleMetadataElement>("label", "person");
    PixelMetadataSpecification selection("labels", metadataSelection);

    std::vector<bool> usesOneTiles{true, false};
    for (const auto& suffix : suffixes) {
        for (bool usesOnlyOneTile : usesOneTiles) {
            for (int i = 0; i < 3; ++i) {
