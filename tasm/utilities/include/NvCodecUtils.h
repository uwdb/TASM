...
inline bool check(HRESULT e, int iLine, const char *szFile) {
    if (e != S_OK) {
        std::cerr << "HRESULT error 0x" << (void *)e << " at line " << iLine << " in file " << szFile << std::endl;
        return false;
    }
    return true;
}
    ...
    BufferedFileReader(const char *szFileName, bool bPartial = false) {
        ...
        while (nSize) {
            try {
                ...
                if (nSize != st.st_size) {
                    std::cerr << "File is too large - only " << std::setprecision(4) << 100.0 * nSize / st.st_size << "% is loaded" << std::endl;
                }
                break;
            } catch(std::bad_alloc) {
                if (!bPartial) {
